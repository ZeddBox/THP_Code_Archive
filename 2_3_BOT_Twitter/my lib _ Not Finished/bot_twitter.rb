...
# client.update('#bonjour_monde @the_hacking_pro que penses-tu de mon bot ? Plutôt stylé non ?')

# ligne qui permet de follow "screen name" / id
# client.follow("matdej4")


# Envoyer un tweet à 5 journalistes
...
def send_tweet_to_n_random_user(list, n=1)
  login_twitter
  i = 0
  while i < n
    desti = list.shuffle[n]
    $client.update("#JeSuisUnBOT en formation @the_hacking_pro ! #{desti} RESTEZ CHEZ VOUS !")
    i += 1
  end
end
## ==> send_tweet_to_n_random_user(journalist, 5)

# Liker les n derniers tweets avec XX dedans
def like_n_last_tweets_with(str, n)
  tweets = $client.search(str, result_type: "recent").take(20)
  $client.favorite(tweets)
end
## ==> like_n_last_tweets_with("#bonjour_monde", 10)

# Search and follow the n last guys using str
def searchAdd(topics, n)
  $client.search(topices, result_type: "recent").take(n).collect do |tweet|
    $client.follow("#{tweet.user.screen_name}")
  end
end
## ==> searchAdd("#bonjour_monde", 10)
